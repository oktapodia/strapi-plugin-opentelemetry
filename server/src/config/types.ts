...
export type NormalizeConfig =
  | NormalizeRule[]
  | ((path: string, ctx: { method: string }) => string);
...
export interface ResolvedOpenTelemetryConfig {
  ...
  resourceAttributes: Record<string, string>;
  diagLogLevel: DiagLogLevelName;
  traces: Required<Pick<TracesConfig, 'enabled' | 'exporter' | 'protocol'>> &
    Omit<TracesConfig, 'enabled' | 'exporter' | 'protocol'>;
  metrics: Required<
    Pick<
      MetricsConfig,
      | 'enabled'
  ...
  > &
    Omit<
      ...
      | 'lifecycleMetrics'
    >;
  logs: Required<Pick<LogsConfig, 'enabled' | 'exporter' | 'protocol'>> &
    Omit<LogsConfig, 'enabled' | 'exporter' | 'protocol'>;
  instrumentations: Required<InstrumentationsConfig>;
  http: Required<Pick<HttpMetricsConfig, 'enabled' | 'ignorePaths' | 'recordPayloadSizes'>> &
    Pick<HttpMetricsConfig, 'normalize'>;
