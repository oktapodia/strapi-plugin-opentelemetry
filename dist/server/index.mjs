...
import { DiagLogLevel, trace, metrics, diag, DiagConsoleLogger } from "@opentelemetry/api";
...
import { OTLPLogExporter as OTLPLogExporter$1 } from "@opentelemetry/exporter-logs-otlp-http";
import { OTLPMetricExporter } from "@opentelemetry/exporter-metrics-otlp-grpc";
import { OTLPMetricExporter as OTLPMetricExporter$1 } from "@opentelemetry/exporter-metrics-otlp-http";
...
const buildInstrumentations = (config2, metrics2) => {
  if (!config2.enabled) {
    return [];
  }
...
const createTraceExporter = (protocol, config2) => {
  ...
  return new OTLPTraceExporter$1(options);
};
const createMetricExporter = (protocol, config2) => {
  ...
  return new OTLPMetricExporter$1(options);
};
const createLogExporter = (protocol, config2) => {
  ...
  return new OTLPLogExporter$1(options);
};
...
const buildResource = (config2) => {
  const attributes = {
    ...
    [ATTR_SERVICE_INSTANCE_ID]: config2.serviceInstanceId,
    ...config2.resourceAttributes
  };
  if (config2.serviceVersion) {
    attributes[ATTR_SERVICE_VERSION] = config2.serviceVersion;
  }
...
const initializeTelemetry = async (config2, logger) => {
  ...
  try {
    ...
    if (config2.metrics.hostMetrics && config2.metrics.enabled && config2.metrics.exporter === "otlp") {
      const hostMetrics = new HostMetrics({ name: config2.serviceName });
      hostMetrics.start();
...
const getMeter = (name = "strapi-plugin-opentelemetry", version = "0.1.1") => {
  return metrics.getMeter(name, version);
};
const getTracer = (name = "strapi-plugin-opentelemetry", version = "0.1.1") => {
...
const bootstrap = ({ strapi }) => {
  ...
  if (!config2.enabled || !isTelemetryInitialized()) {
    return;
  }
  if (config2.http.enabled && config2.metrics.httpMetrics) {
